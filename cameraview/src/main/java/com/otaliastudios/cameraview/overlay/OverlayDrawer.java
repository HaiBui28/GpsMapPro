...
public class OverlayDrawer {
    ...
    public void draw(@NonNull Overlay.Target target) {
        if (!overlayDirty) return; // Không cần vẽ lại nếu không có thay đổi
        ...
        try {
            ...
            // Đánh dấu đã vẽ xong, không cần vẽ lại cho đến khi có thay đổi mới
            overlayDirty = false;

        } catch (Surface.OutOfResourcesException e) {
            LOG.w("Surface out of resources", e);
        }
        ...
        synchronized (mIssue514WorkaroundLock) {
            ...
            try {
                ...
                LOG.w("IllegalStateException updating texture", e);
            }
        }
